...
2. **AUD-2026-002（缺材料单）**：阿姨档案缺少身份证号，办结时被拦截返回 ERR_MISSING_EVIDENCE
3. **AUD-2026-003（逾期单）**：到期日 2026-06-05（已过期），出现在逾期队列，当前处理人为服务督导
4. **AUD-2026-004（退回补正单）**：状态为待补正，审计备注记录了退回原因"资质材料不完整"，异常原因"健康证过期需更新"
5. **AUD-2026-005（临期+状态冲突单）**：到期日 2026-06-18（临期），状态为复核中，非城市经理角色操作会返回 ERR_INVALID_STATUS
6. **AUD-2026-006（逾期+待处理单）**：到期日 2026-06-10（已过期），状态为待处理，出现在逾期队列

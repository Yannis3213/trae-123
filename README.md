...
| 账号 | 角色 | 岗位名称 | 权限 |
| --- | --- | --- | --- |
| `consultant` | 课程顾问（registrar） | **培训项目登记员** | 创建/编辑/补正/提交审核/推进阶段/附件管理 |
| `trainer_ops` | 讲师运营（auditor） | **培训项目审核主管** | 过程核对：审核通过 / 退回补正（退回原因必填） |
| `project_mgr` | 项目经理（reviewer） | **企业培训公司复核负责人** | 结果确认：复核通过同步 / 复核退回 / 归档 |
...
| 必填证据 | 提交/审核/复核/**补正提交** | `missing_evidence`（EVI-SUP-001） | ✅ |
| 退回原因必填 | `audit_reject` / `review_reject` | 纯拦截（EVI-RJ-001） | ✅ |
| 截止日期 | 每次办理 + 加载详情 | `overdue`（OVERDUE-001）+ 节点超时（DUE-XXX-001） | ✅ |
...
- 顶部 4 格关键信息（阶段 / 状态 / 到期 / 当前处理人 / 未处理异常角标）
- **当前角色可用操作区**：只显示角色 + 当前状态允许的动作
  - 退回类动作强制填写退回原因（后端 EVI-RJ-001 拦截）
  - 缺材料动作挂「⚠缺 N」角标
  - 每次操作记录版本号
- **六个 Tab**（完整的退回补正与异常追溯链路）：
  1. **📋 基本信息**：可编辑字段（仅课程顾问在草稿/退回补正状态可改）
  2. **🔧 补正办理**：退回原因历史 + 待补正清单（每项带「立即填写/上传」快捷跳转）+ 补正一键提交
  3. **📎 附件材料**：阶段齐全度提示 + 必备证据 red badge + 需求/方案/合同 阶段分类 badge + 附件新增
  4. **📝 处理记录**：时间轴 + 责任岗位中文标签 + 状态/阶段流转 + 证据核对说明
  5. **⚠️ 异常追溯**（独立 Tab）：异常类型 / 编码 / 原因 / 责任岗位 / 责任人 / 已处理状态
  6. **🔍 审计备注**：三色分类卡片（状态变更/补正/异常）
...
后端从 `.env` 读取 `FRONTEND_PORT=3106`、`BACKEND_PORT=8106`，前端请求地址、后端监听端口、CORS 白名单、前端 `app.config.ts` 的 API Base **全部唯一**，没有多余端口配置。

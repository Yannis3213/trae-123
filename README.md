...
系统实现完整的跨角色交接追踪，确保工单在不同角色间流转时责任清晰、轨迹可查。

### 下一处理人自动分派规则

| 当前状态 | 操作 | 目标状态 | 下一处理人 | 说明 |
|---|---|---|---|---|
| 待签收 / 异常回传 | 签收登记 | 来电登记 | 操作人（登记员/坐席） | 登记后由登记者负责 |
| 来电登记 | 派单处理 | 问题派单 | 客服审核主管（王芳） | 派单后由主管负责审核 |
| 问题派单（紧急/高优） | 质检签收 | 签收完成 | 复核负责人（张伟） | 紧急工单优先质检 |
| 问题派单 | 回访关闭 | 回访关闭 | 操作人（坐席/登记员） | 回访后待复核归档 |
| 问题派单 | 审核签收 | 签收完成 | 复核负责人（张伟） | 签收后待复核归档 |
| 回访关闭 / 签收完成 | 复核归档 | 已归档 | 操作人（复核负责人） | 归档后流程结束 |
| 任意状态 | 退回补正 | 异常回传 | 客服登记员（李明） | 退回后由原登记员补正 |

### 交接轨迹展示

- **工单列表**：新增「下一处理人」列，预判展示下一步将由谁处理
- **工单详情**：顶部「交接流转信息」卡片显示当前处理人 → 下一处理人
- **处理记录**：每条记录显示 `📤 交接：张三 → 李四`，完整还原责任流转
- **批量结果**：每条成功记录显示回写后的新状态、新处理人；失败记录分类显示失败原因（版本冲突/状态冲突/缺少证据/越权操作）

### SQLite 持久化字段

`processing_records` 表新增交接追踪字段：
- `from_handler_id` / `from_handler_name`：交接前处理人
- `to_handler_id` / `to_handler_name`：交接后处理人

`tickets` 表状态回写字段：
- `current_handler_id` / `current_handler_name`：实时更新当前责任人
- `next_handler_id` / `next_handler_name`：列表预判显示下一处理人
...
- `processing_records`：动作、from→to 状态、**from→to 处理人**、操作人、角色、备注

...
| 订单号 | 状态 | 说明 | 对应试用场景 |
|--------|------|------|--------------|
| `TO-20260611-001` | **草稿 (draft)** | 旅游登记员刚创建的北京五日游草稿，已核验线路报价 | 正常流转：草稿→提交审核→审核通过→复核归档 |
| `TO-20260611-002` | **待审核 (pending_audit)** | 上海苏杭七日游，含线路报价与报名确认证据，1 天内临期 | 临期预警：1 天内到期，审核时如超时则逾期 |
| `TO-20260611-003` | **待补正 (pending_correction)** | 云南六日游，已逾期 1 天，异常原因 + 审计备注 + 处理记录齐全 | 超时/逾期：无法直接推进审核/复核，需登记员补正 |
| `TO-20260611-004` | **待复核 (pending_review)** | 海南三亚五日游，三类证据齐全，版本 v4 | 退回补正或状态冲突：复核退回后回到待补正，登记员需重新提交 |
| `TO-20260611-005` | **待补正 (pending_correction)** | **西藏九日游（复核退回场景）**，从待复核退回，含完整 4 条处理记录 + 双角色审计备注 + 高反健康告知书异常原因 | 完整退回链路：草稿→审核→复核→退回；批量推进时拦截缺失证据 |
| `TO-20260611-006` | **已归档 (archived)** | **泰国跟团游（已归档场景）**，完整 4 条处理记录 + 双角色审计备注 + 三类证据齐全 | 完整归档场景，时间线显示"草稿→审核→复核→归档"全链路 |

...
| YT20240601005 | 待补正 | **新建登记** | 刚创建的订单，已填基础信息无附件，可测试上传委托单后提交 |
...
2. **订单列表**（运输订单登记 / 过程核验 / 复核归档 三个入口）：
   - **新建订单**（仅运输订单登记页、客服专员可见）：弹出表单填写运输委托字段、截止时间、责任人、优先级，提交后刷新列表并跳转详情页继续上传证据
   - **列表摘要**：每行订单号下方显示异常数量、证据缺口、最近处理记录、审计备注数量
   - 筛选：责任人、优先级、截止时间、状态

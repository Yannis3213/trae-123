...
#### 四类试用场景订单（含完整处理记录+审计备注+异常原因）

| 订单号 | 状态 | 场景说明 | 处理记录（processing_records） | 审计备注（audit_notes） | 异常原因 / 责任人队列 |
|--------|------|----------|--------------------------------|------------------------|----------------------|
| `TO-20260611-001` | **草稿 (draft)** | 正常流转起点，v1 | 1 条：登记员创建草稿 | 无 | 责任人：登记员（draft） |
| `TO-20260611-002` | **待审核 (pending_audit)** | **缺证据 + 临期**：线路报价+报名确认齐全，出团审核证据缺失，1 天临期 | 3 条：草稿→提交审核（登记员，含 note）+ **审核主管检查留痕（不推进状态）**（含 note + exception_reason） | 2 条（审核主管）：临期预警提醒 + 缺证据通知 | exception_reason="缺出团审核证据：旅行社行程确认单 / 出团通知书未上传"；责任人：审核组 |
| `TO-20260611-003` | **待补正 (pending_correction)** | **逾期**：已逾期 1 天，审核主管退回补正 | 3 条：草稿→提交审核→退回补正（审核主管，含 exception_reason+note） | 1 条（审核主管）：逾期订单优先处理提醒 | exception_reason="出团日期临近…"；correction_note="请 24 小时内补正行程细节"；责任人：登记员 |
| `TO-20260611-004` | **待复核 (pending_review)** | 三类证据齐全，v4，待复核 | 3 条：草稿→提交审核→审核通过（审核主管，含 note） | 1 条（复核负责人）：三类证据齐全，24h 内完成归档 | 责任人：复核组 |
| `TO-20260611-005` | **待补正 (pending_correction)** | **复核退回（完整链路）**：v5 西藏九日游，从待复核退回 | 4 条：草稿→提交审核→审核通过（审核主管）→**复核退回（复核负责人，含 exception_reason+note）** | 2 条：审核主管备注"已提醒高反注意"+ 复核负责人备注"发现高反告知书+保险+酒店函缺失已退回" | exception_reason="缺少高反健康告知书和保险单…"；correction_note="48 小时内补正高反健康告知书+旅游意外险+拉萨酒店确认函"；责任人：登记员 |
| `TO-20260611-006` | **已归档 (archived)** | **完整归档**：v6 泰国跟团游，三单齐全 | 4 条：草稿→提交审核→审核通过（审核主管 note）→**复核通过归档（复核负责人 note）** | 2 条：审核主管备注"签证机票酒店出团通知齐全"+ 复核负责人备注"三单齐全，客户签字回传完整" | correction_note="已归档：三类证据齐全…"；责任人：已归档 None |

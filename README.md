...
| 用户名 | 角色 | 权限 |
|--------|------|------|
| `community_worker` | 社区专干 | 创建申请、补正材料 |
| `street_clerk` | 街道科员 | 接单、处理、流转、退回 |
| `leader` | 分管领导 | 接单、审批、复核 |
...
| BZDEMO0002 | 缺材料 | 入户核实 | 已接单 | 缺少走访记录、照片证据，尝试流转会被拦截 |
| BZDEMO0003 | 超时逾期 | 救助确认 | 待接单 | 已逾期，预警状态为「逾期」 |
| BZDEMO0004 | 退回补正 | 困难帮扶 | 退回补正 | 缺少困难证明，需社区专干补正 |
| BZDEMO0005 | 临期预警 | 入户核实 | 待接单 | 即将到期，预警状态为「临期」 |
| BZDEMO0006 | 正常办理 | 困难帮扶 | 已接单 | 材料齐全，可正常处理 |
...
### 1. 列表筛选
- 按状态标签筛选：待接单、已接单、验收通过
- 高级筛选：节点、预警状态、社区、关键词搜索
- 支持多条件组合查询

### 2. 详情办理
- 完整展示申请信息、附件材料、处理记录、审计备注
- 根据角色和状态显示可执行操作
- 提交处理结果时实时校验权限
...
2. **缺材料测试**
   - 登录 `street_clerk` → 已接单列表 → BZDEMO0002 → 处理流转
   - 系统提示缺少「走访记录、照片证据」

3. **逾期测试**
   - 登录 `leader` → 待接单列表 → BZDEMO0003
   - 预警标签显示「逾期」红色标记

4. **退回补正测试**
   - 登录 `community_worker` → 查看 BZDEMO0004
   - 状态为「退回补正」，可执行「补正提交」
...
5. **证据校验**：流转到下一节点时必填证据是否齐全
6. **防重复提交**：5秒内相同操作视为重复提交
...
3. 所有端口配置统一，请勿修改 `3106` 和 `8106`
4. CORS 白名单仅允许 `http://localhost:3106` 访问

...
func SeedDemoData() {
	...
	reg2User, _ := repository.GetUserByUsername("registrar2")
	audUser, _ := repository.GetUserByUsername("auditor1")
	aud2User, _ := repository.GetUserByUsername("auditor2")
	revUser, _ := repository.GetUserByUsername("reviewer1")
	rev2User, _ := repository.GetUserByUsername("reviewer2")
	if regUser == nil || reg2User == nil || audUser == nil || aud2User == nil || revUser == nil || rev2User == nil {
		return
	}
	...
	demos := []struct {
		...
		},
		{
			c: models.Consultation{
				...
				Gender:             "男",
				Department:         "普外科",
				AttendingPhysician: "何医生",
				ConsultationType:   "科间会诊",
				ConsultationReason: "腹痛待查，怀疑急性阑尾炎，需普外科会诊评估是否急诊手术",
				ConsultationDept:   "普外科",
				RequestedDoctor:    "何主任",
				Urgency:            config.UrgencyWarning,
				Status:             config.StatusPending,
				CurrentStage:       config.StageRegistration,
				CurrentHandler:     reg2User.ID,
				RegistrarID:        reg2User.ID,
				Version:            1,
				EvidenceList:       "病历,血常规,腹部B超",
				CreatedBy:          reg2User.ID,
				UpdatedBy:          reg2User.ID,
				Deadline:           timePtr(now.Add(10 * time.Hour)),
			},
			processRecords: []models.ProcessRecord{
				{
					Stage:       config.StageRegistration,
					Action:      "create",
					ToStatus:    config.StatusPending,
					HandlerID:   reg2User.ID,
					HandlerName: reg2User.RealName,
					HandlerRole: reg2User.Role,
					Remark:      "创建腹痛待查会诊申请",
					Version:     1,
				},
			},
			notes: []string{"孙秘书登记中，月底集中处理-对照单据（registrar2）"},
		},
		{
			c: models.Consultation{
				PatientName:        "孙八",
				...
				Gender:             "男",
				Department:         "骨科",
				AttendingPhysician: "罗医生",
				ConsultationType:   "科间会诊",
				ConsultationReason: "右膝关节置换术后第5天，右下肢肿胀伴D-二聚体升高，怀疑深静脉血栓，需血管外科会诊",
				ConsultationDept:   "血管外科",
				RequestedDoctor:    "黄主任",
				Urgency:            config.UrgencyNormal,
				Status:             config.StatusAbnormal,
				CurrentStage:       config.StageVerification,
				CurrentHandler:     aud2User.ID,
				RegistrarID:        reg2User.ID,
				AuditorID:          aud2User.ID,
				Version:            2,
				EvidenceList:       "手术记录,血常规,凝血功能",
				CreatedBy:          reg2User.ID,
				UpdatedBy:          aud2User.ID,
				Deadline:           timePtr(now.Add(20 * time.Hour)),
			},
			processRecords: []models.ProcessRecord{
				{
					Stage:       config.StageRegistration,
				...
				},
				{
					...
					FromStatus:   config.StatusPending,
					ToStatus:     config.StatusAbnormal,
					HandlerID:    aud2User.ID,
					HandlerName:  aud2User.RealName,
					HandlerRole:  aud2User.Role,
					Remark:       "核验异常：缺少下肢血管超声报告",
					EvidenceUsed: "手术记录,血常规",
					Version:      2,
				},
			},
			abnormals: []models.AbnormalRecord{
				{
					AbnormalType: "缺证据",
					Reason:       "缺少下肢血管超声检查，深静脉血栓诊断依据不足，请登记员补充",
					ReportedBy:   aud2User.ID,
					IsResolved:   false,
				},
			},
			notes: []string{"对照单据：周质控核验异常，孙秘书创建"},
		},
		{
			c: models.Consultation{
				PatientName:        "周九",
				...
				Gender:             "女",
				Department:         "呼吸内科",
				AttendingPhysician: "许医生",
				ConsultationType:   "多学科会诊",
				ConsultationReason: "慢性阻塞性肺疾病急性加重合并II型呼衰，需呼吸科、营养科、康复科多学科协作制定方案",
				ConsultationDept:   "呼吸科,营养科,康复科",
				RequestedDoctor:    "郑主任,冯主任,陈主任",
				Urgency:            config.UrgencyNormal,
				Status:             config.StatusArchived,
				CurrentStage:       config.StageReview,
				CurrentHandler:     rev2User.ID,
				RegistrarID:        reg2User.ID,
				AuditorID:          aud2User.ID,
				ReviewerID:         rev2User.ID,
				IsArchived:         true,
				Version:            4,
				EvidenceList:       "病历,肺功能报告,动脉血气,胸片,营养评估",
				CreatedBy:          reg2User.ID,
				UpdatedBy:          rev2User.ID,
			...
			processRecords: []models.ProcessRecord{
				...
				},
				{
					...
					HandlerName:  aud2User.RealName,
					HandlerRole:  aud2User.Role,
					Remark:       "质控核验通过，多学科排班已确认",
					EvidenceUsed: "病历,肺功能报告,动脉血气,胸片",
					Version:      2,
				},
				{
					Stage:        config.StageReview,
					Action:       "review_pass",
					FromStatus:   config.StatusRechecked,
					ToStatus:     config.StatusRechecked,
					HandlerID:    rev2User.ID,
					HandlerName:  rev2User.RealName,
					HandlerRole:  rev2User.Role,
					Remark:       "医务部复核通过，多学科会诊意见已反馈",
					EvidenceUsed: "病历,肺功能报告,动脉血气,胸片,营养评估",
					Version:      3,
				},
				{
					Stage:        config.StageReview,
					Action:       "archive",
					FromStatus:   config.StatusRechecked,
					ToStatus:     config.StatusArchived,
					HandlerID:    rev2User.ID,
					HandlerName:  rev2User.RealName,
					HandlerRole:  rev2User.Role,
					Remark:       "医务部吴主任完成最终归档",
			...
			notes: []string{"对照归档单据：孙秘书创建，周质控核验，吴主任复核归档"},
		},
	}

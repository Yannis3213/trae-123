...
pub async fn init_database(pool: &SqlitePool) -> Result<(), sqlx::Error> {
    ...
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_cases_status ON cases(status)").execute(pool).await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_cases_stage ON cases(current_stage)").execute(pool).await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_cases_deadline ON cases(deadline)").execute(pool).await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_attachments_case ON attachments(case_id)").execute(pool).await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_records_case ON processing_records(case_id)").execute(pool).await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_notes_case ON audit_notes(case_id)").execute(pool).await?;
...
pub async fn seed_demo_data(pool: &SqlitePool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    sqlx::query(r#"INSERT INTO users (id, username, real_name, role, password_hash, created_at, updated_at) VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"#)
        ...
        .execute(pool).await?;
    sqlx::query(r#"INSERT INTO users (id, username, real_name, role, password_hash, created_at, updated_at) VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"#)
        .bind(officer_id.to_string()).bind("officer").bind("李主管").bind(Role::PoliceOfficer.as_str()).bind(&officer_pw)
        .execute(pool).await?;
    sqlx::query(r#"INSERT INTO users (id, username, real_name, role, password_hash, created_at, updated_at) VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"#)
    ...
    let cases = vec![
        ...
        ("JQ202606005", "退回补正-诈骗案证据不足", "受害人被电信诈骗2万元，已止付。", "刑事案件", "线上/电话诈骗", "孙七", "13800138005", CaseStatus::PendingCorrection, ProcessingStage::Registration, false, true, false, now + Duration::days(7), 3, dispatcher_id, "张登记", None, None),
        ("JQ202606006", "复核中案件-寻衅滋事待所领导审批", "某工地工人因工资问题围堵项目部。", "治安案件", "城东工地项目部", "周八", "13800138006", CaseStatus::UnderReview, ProcessingStage::Review, true, true, true, now + Duration::days(3), 4, dispatcher_id, "张登记", Some(reviewer_id), Some("王所长".to_string())),
        ("JQ202606007", "状态冲突测试-已办结但材料不全", "遗失物品求助。", "求助", "人民公园", "吴九", "13800138007", CaseStatus::UnderReview, ProcessingStage::Dispatch, false, true, false, now + Duration::days(4), 2, dispatcher_id, "张登记", Some(officer_id), Some("李主管".to_string())),
    ...
    for (case_number, title, description, case_type, location, reporter_name, reporter_phone, status, current_stage, reg_complete, dispatch_met, followup_complete, deadline, version, created_by, created_by_name, handler_id, handler_name) in cases {
        ...
        let completed_at = if status == CaseStatus::Completed { Some((now - Duration::hours(2)).to_rfc3339()) } else { None };

        sqlx::query(
            r#"INSERT INTO cases (id, case_number, title, description, case_type, location, reporter_name, reporter_phone, status, current_stage, current_handler_id, current_handler_name, registration_materials_complete, dispatch_timeline_met, followup_evidence_complete, deadline, version, created_by, created_by_name, created_at, updated_at, completed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, ?)"#,
        )
        .bind(case_id.to_string()).bind(case_number).bind(title).bind(description).bind(case_type).bind(location)
        .bind(reporter_name).bind(reporter_phone).bind(status.as_str()).bind(current_stage.as_str())
        .bind(handler_id.map(|id| id.to_string())).bind(handler_name)
        .bind(reg_complete as i32).bind(dispatch_met as i32).bind(followup_complete as i32)
        .bind(deadline.to_rfc3339()).bind(version).bind(created_by.to_string()).bind(created_by_name)
        .bind(completed_at)
        .execute(pool).await?;
        ...
        if reg_complete {
            ...
            if case_number != "JQ202606007" {
                ...
                sqlx::query(r#"INSERT INTO attachments (id, case_id, file_name, file_type, file_size, category, uploaded_by, uploaded_by_name, uploaded_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)"#)
                    .bind(attach2_id.to_string()).bind(case_id.to_string()).bind(format!("{}_现场照片.jpg", case_number))
                    .bind("image/jpeg").bind(1024000i64).bind("evidence").bind(officer_id.to_string()).bind("李主管")
                    .execute(pool).await?;
            }
        }
        ...
        if followup_complete && case_number != "JQ202606007" {
            ...
            sqlx::query(r#"INSERT INTO attachments (id, case_id, file_name, file_type, file_size, category, uploaded_by, uploaded_by_name, uploaded_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)"#)
                .bind(attach3_id.to_string()).bind(case_id.to_string()).bind(format!("{}_回访录音.mp3", case_number))
                .bind("audio/mpeg").bind(5120000i64).bind("followup").bind(officer_id.to_string()).bind("李主管")
                .execute(pool).await?;
        }
        ...
        if case_number == "JQ202606004" || case_number == "JQ202606009" {
            ...
            sqlx::query(r#"INSERT INTO audit_notes (id, case_id, note, anomaly_reason, noted_by, noted_by_name, noted_at) VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)"#)
                .bind(note_id.to_string()).bind(case_id.to_string()).bind(format!("逾期预警：{}", reason)).bind(reason).bind(reviewer_id.to_string()).bind("王所长")
                .execute(pool).await?;
        }
    }
...
async fn seed_processing_records(
    ...
    let records: Vec<(ProcessingStage, &str, Option<CaseStatus>, CaseStatus, Uuid, &str, Role, &str)> = match (case_number, status, stage) {
        (_, CaseStatus::Completed, _) => vec![
            (ProcessingStage::Registration, "接警登记", None, CaseStatus::UnderReview, dispatcher_id, dispatcher_name, Role::Dispatcher, "警情登记完成，材料齐全，移交民警处理"),
            (ProcessingStage::Dispatch, "民警处置", Some(CaseStatus::UnderReview), CaseStatus::UnderReview, officer_id, officer_name, Role::PoliceOfficer, "已完成现场处置，派警及时，回访确认证据齐全"),
            (ProcessingStage::Review, "所领导复核", Some(CaseStatus::UnderReview), CaseStatus::Completed, reviewer_id, reviewer_name, Role::Reviewer, "材料完整、处置及时、回访到位，予以办结"),
        ],
        (c, CaseStatus::PendingCorrection, _) if c == "JQ202606002" || c == "JQ202606005" => vec![
            (ProcessingStage::Registration, "接警登记", None, CaseStatus::UnderReview, dispatcher_id, dispatcher_name, Role::Dispatcher, "初步登记完成，等待审核"),
            (ProcessingStage::Registration, "退回补正", Some(CaseStatus::UnderReview), CaseStatus::PendingCorrection, reviewer_id, reviewer_name, Role::Reviewer, "材料不齐全，退回登记员补充"),
        ],
        (_, CaseStatus::UnderReview, ProcessingStage::Dispatch) => vec![
            (ProcessingStage::Registration, "接警登记", None, CaseStatus::UnderReview, dispatcher_id, dispatcher_name, Role::Dispatcher, "警情登记完成，移交民警处理"),
            (ProcessingStage::Dispatch, "民警处置中", Some(CaseStatus::UnderReview), CaseStatus::UnderReview, officer_id, officer_name, Role::PoliceOfficer, "正在开展调查取证工作"),
        ],
        (_, CaseStatus::UnderReview, ProcessingStage::Review) => vec![
            (ProcessingStage::Registration, "接警登记", None, CaseStatus::UnderReview, dispatcher_id, dispatcher_name, Role::Dispatcher, "警情登记完成，材料齐全"),
            (ProcessingStage::Dispatch, "民警处置", Some(CaseStatus::UnderReview), CaseStatus::UnderReview, officer_id, officer_name, Role::PoliceOfficer, "处置完成，派警及时，证据齐全，移交复核"),
        ],
        _ => vec![],
    };

    for (stg, action, from_status, to_status, handler_id, handler_name, handler_role, remarks) in records {
        let record_id = Uuid::new_v4();
        sqlx::query(
            r#"INSERT INTO processing_records (id, case_id, stage, action, from_status, to_status, handler_id, handler_name, handler_role, remarks, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)"#,
        )
        .bind(record_id.to_string()).bind(case_id.to_string()).bind(stg.as_str()).bind(action)
        .bind(from_status.map(|s| s.as_str())).bind(to_status.as_str())
        .bind(handler_id.to_string()).bind(handler_name).bind(handler_role.as_str()).bind(remarks)
        .execute(pool).await?;
    }

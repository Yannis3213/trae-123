...
export default async function (fastify, opts) {
  ...
  const addComputedFields = (plan) => {
    ...
    return {
      ...
      current_handler_role: plan.status === STATUSES.DRAFT
        ? (plan.owner === plan.current_handler ? ROLES.CS_MANAGER : ROLES.DELIVERY_CONSULTANT)
        : ROLES.CS_LEAD,
    };
  };
  ...
  fastify.get('/api/launch-plans', {
    ...
    handler: async (req, reply) => {
      ...
      const result = warning
        ? enriched.filter(p => p.deadline_warning === warning)
        : enriched;
  ...
  fastify.post('/api/launch-plans', {
    schema: {
      body: {
        ...
        properties: {
          ...
          owner: { type: 'string' },
          launch_target: { type: 'string' },
          config_checklist: { type: 'string' },
    ...
    handler: async (req, reply) => {
      ...
      `).run(
        ...
        STATUSES.DRAFT, body.owner, body.owner,
      ...
      `).run(
        uuidv4(), id, 'create', '', STATUSES.DRAFT, user.name, user.role,
        '创建上线计划单', '', ts
      );
  ...
  fastify.put('/api/launch-plans/:id', {
    ...
    handler: async (req, reply) => {
      ...
      if (existing.version !== version) {
        logException(req.params.id, 'version_conflict',
          `提交版本${version}与后端版本${existing.version}冲突，用户：${user.name}`, user.name);
        reply.status(409);
        return {
          error: '版本冲突：该单据已被他人修改，请刷新后重试',
          current_version: existing.version,
        };
      }

      if (existing.status === STATUSES.ARCHIVED) {
        reply.status(400);
        return { error: '已归档的单据不能修改' };
      }

      if (existing.current_handler !== user.name
        && user.role !== ROLES.CS_LEAD
        && !(user.role === ROLES.CS_MANAGER && existing.owner === user.name)) {
        reply.status(403);
        return { error: `当前处理人应为${existing.current_handler}，或由负责人修改` };
      }

      const allowedFields = [
        'customer_name', 'project_name', 'priority', 'deadline', 'launch_target',
        'config_checklist', 'acceptance_notes', 'result', 'reject_reason'
      ];
      const sets = [];
      const values = [];
      ...
      if (!checkRolePermission(user, [ROLES.CS_MANAGER, ROLES.DELIVERY_CONSULTANT, ROLES.CS_LEAD])) {
        reply.status(403);
        return { error: '当前角色无权限修改' };
      }
      ...
      `).run(
        uuidv4(), req.params.id, 'update', existing.status, existing.status,
        user.name, user.role, '更新上线计划单内容', '', now()
      );
  ...
  fastify.post('/api/launch-plans/:id/submit', {
    ...
    handler: async (req, reply) => {
      ...
      if (existing.version !== version) {
        logException(req.params.id, 'version_conflict',
          `提交审核版本冲突，提交版本${version} vs 后端版本${existing.version}`, user.name);
        reply.status(409);
        return { error: '版本冲突，请刷新后重试' };
      }

      if (existing.status !== STATUSES.DRAFT) {
        reply.status(400);
        return { error: `当前状态为${STATUS_NAMES[existing.status]}，不能提交审核` };
      }

      if (existing.current_handler !== user.name && existing.owner !== user.name) {
        reply.status(403);
        return { error: `当前处理人应为${existing.current_handler}或负责人${existing.owner}` };
      }
      ...
      if (missing.length > 0) {
        logException(req.params.id, 'missing_evidence',
          `提交时缺少必要证据: ${missing.join('、')}`, user.name);
        reply.status(400);
        return { error: `提交前请补齐材料：${missing.join('、')}` };
      }
      ...
      if (attachCount < 1 && !evidence) {
        logException(req.params.id, 'missing_evidence',
          '提交审核缺少附件或证据说明', user.name);
        reply.status(400);
        return { error: '请至少上传一个附件或填写证据说明' };
      }

      const ts = now();
      db.prepare(`
        UPDATE launch_plans
        SET status = ?, current_handler = ?, version = ?, updated_at = ?, reject_reason = ''
        WHERE id = ?
      `).run(STATUSES.PENDING_REVIEW, '王总', existing.version + 1, ts, req.params.id);
      ...
      `).run(
        uuidv4(), req.params.id, 'submit', STATUSES.DRAFT, STATUSES.PENDING_REVIEW,
  ...
  fastify.post('/api/launch-plans/:id/reject', {
    schema: {
      body: {
        type: 'object',
        required: ['version', 'reject_reason'],
        properties: {
          version: { type: 'integer' },
          reject_reason: { type: 'string', minLength: 5 },
          comment: { type: 'string' },
        },
      },
    },
    handler: async (req, reply) => {
      ...
      if (existing.status !== STATUSES.PENDING_REVIEW) {
        reply.status(400);
        return { error: `当前状态为${STATUS_NAMES[existing.status]}，不能退回` };
      }

      const ts = now();
      db.prepare(`
        UPDATE launch_plans
        SET status = ?, current_handler = ?, reject_reason = ?, version = ?, updated_at = ?
        WHERE id = ?
      `).run(STATUSES.DRAFT, existing.owner, reject_reason, existing.version + 1, ts, req.params.id);
      ...
      `).run(
        uuidv4(), req.params.id, 'reject', STATUSES.PENDING_REVIEW, STATUSES.DRAFT,
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `).run(
        uuidv4(), req.params.id, `退回原因：${reject_reason}。${comment || ''}`,
        user.name, user.role, ts
      );

      return { message: '已退回补正', new_version: existing.version + 1 };
    },
  });
  ...
  fastify.post('/api/launch-plans/:id/archive', {
    ...
    handler: async (req, reply) => {
      ...
      if (existing.status !== STATUSES.PENDING_REVIEW) {
        reply.status(400);
        return { error: `当前状态为${STATUS_NAMES[existing.status]}，不能归档` };
      }

      if (!existing.acceptance_notes || existing.acceptance_notes.trim().length < 10) {
        reply.status(400);
        return { error: '归档前验收确认内容不能为空或不完整' };
      }

      if (!result || result.trim().length < 5) {
        reply.status(400);
        return { error: '请填写处理结果后归档' };
      }

      const ts = now();
      db.prepare(`
        UPDATE launch_plans
        SET status = ?, result = ?, version = ?, updated_at = ?
        WHERE id = ?
      ...
      `).run(
        uuidv4(), req.params.id, 'archive', STATUSES.PENDING_REVIEW, STATUSES.ARCHIVED,
    ...
    handler: async (req, reply) => {
      ...
      const tx = db.transaction(() => {
        for (const id of ids) {
          ...
          if (!plan) {
            results.push({ id, success: false, reason: '单据不存在' });
            continue;
          }

          const warning = getDeadlineWarning(plan.deadline, plan.status);
          if (warning === 'overdue') {
            logException(id, 'overdue_blocked',
              `批量推进拦截：单据已逾期，责任人${plan.owner}`, user.name);
            db.prepare(`
              INSERT INTO process_records (
                id, launch_plan_id, action, from_status, to_status, operator,
                operator_role, comment, evidence, created_at
            ...
            `).run(
              uuidv4(), id, 'overdue_blocked', plan.status, plan.status,
              user.name, user.role,
              `批量推进拦截：单据已逾期，请在详情页处理补正。责任人：${plan.owner}。截止日期：${plan.deadline}`,
              '逾期拦截', now()
            );
            results.push({
              id,
              ...
              success: false,
              reason: `已逾期拦截：截止日期${plan.deadline}，责任人${plan.owner}，请在详情页补正`,
            });
            continue;
          }
          ...
          if (plan.status === STATUSES.ARCHIVED) {
            results.push({
              ...
              success: false, reason: '已归档单据无需推进',
            });
            continue;
          }
          ...
          if (target_status === STATUSES.PENDING_REVIEW) {
            if (plan.status !== STATUSES.DRAFT) {
              results.push({
                id, plan_no: plan.plan_no, customer_name: plan.customer_name,
                success: false, reason: `当前状态为${STATUS_NAMES[plan.status]}，无法推进到待复核`,
              });
              continue;
            }
            const missing = [];
            ...
            if (missing.length > 0) {
              logException(id, 'missing_evidence',
                `批量推进拦截：缺少${missing.join('、')}`, user.name);
              results.push({
                id, plan_no: plan.plan_no, customer_name: plan.customer_name,
                success: false, reason: `材料不完整：缺少${missing.join('、')}`,
              });
              continue;
            }
            db.prepare(`
              UPDATE launch_plans SET status = ?, current_handler = ?, version = ?, updated_at = ?
              WHERE id = ?
            `).run(STATUSES.PENDING_REVIEW, '王总', plan.version + 1, now(), id);
            db.prepare(`
              INSERT INTO process_records (
                id, launch_plan_id, action, from_status, to_status, operator,
                operator_role, comment, evidence, created_at
            ...
            `).run(
              uuidv4(), id, 'submit', STATUSES.DRAFT, STATUSES.PENDING_REVIEW,
              user.name, user.role, comment || '批量推进到待复核', '批量操作', now()
            );
            results.push({
              id, plan_no: plan.plan_no, customer_name: plan.customer_name,
              success: true, reason: '已提交复核',
            });
          } else if (target_status === STATUSES.ARCHIVED) {
            if (!checkRolePermission(user, [ROLES.CS_LEAD])) {
              results.push({
                id, plan_no: plan.plan_no, customer_name: plan.customer_name,
                success: false, reason: '仅客户成功负责人可批量归档',
              });
              continue;
            }
            if (plan.status !== STATUSES.PENDING_REVIEW) {
              results.push({
                id, plan_no: plan.plan_no, customer_name: plan.customer_name,
                success: false, reason: `当前状态为${STATUS_NAMES[plan.status]}，无法归档`,
              });
              continue;
            }
            if (!plan.acceptance_notes || plan.acceptance_notes.trim().length < 10) {
              results.push({
                id, plan_no: plan.plan_no, customer_name: plan.customer_name,
                success: false, reason: '验收确认不完整，无法归档',
              });
              continue;
            }
            db.prepare(`
              UPDATE launch_plans SET status = ?, result = ?, version = ?, updated_at = ?
              WHERE id = ?
            ...
            `).run(
              uuidv4(), id, 'archive', STATUSES.PENDING_REVIEW, STATUSES.ARCHIVED,
              user.name, user.role, comment || '批量归档', '批量操作', now()
            );
            results.push({
              id, plan_no: plan.plan_no, customer_name: plan.customer_name,
              success: true, reason: '已归档',
            });
          }
        }
      });
      ...
      try {
        ...
        reply.status(500);
        return { error: '批量处理异常：' + err.message };
      }
  ...
  fastify.post('/api/launch-plans/:id/attachments', async (req, reply) => {
    ...
    for await (const part of parts) {
      if (part.filename) {
        ...
        const id = uuidv4();
        db.prepare(`
          INSERT INTO attachments (id, launch_plan_id, file_name, file_path, file_size, uploaded_by, uploaded_at)
          VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    db.prepare(`
      UPDATE launch_plans SET version = version + 1, updated_at = ? WHERE id = ?
  ...
  fastify.delete('/api/attachments/:id', async (req, reply) => {
    ...
    if (plan.status === STATUSES.ARCHIVED) {
      reply.status(400);
      return { error: '已归档单据附件不能删除' };
    }
    if (att.uploaded_by !== user.name && user.role !== ROLES.CS_LEAD) {
      reply.status(403);
      return { error: '仅上传者或负责人可删除附件' };
    }
    ...
    db.prepare(`UPDATE launch_plans SET version = version + 1, updated_at = ? WHERE id = ?`)
      .run(now(), att.launch_plan_id);
    return { success: true };
  });
  ...
  fastify.get('/api/me', async (req, reply) => {
    return {
      name: req.user.name,
      role: req.user.role,
      role_name: ROLE_NAMES[req.user.role] || req.user.role,
    };
  });
}

...
const roleConfig = ROLE_CONFIG[ROLES.REGISTRAR];
...
export default function RegistrationPage() {
  ...
  return (
    <div className="p-6">
      <div className="mb-4">
        <h2 className="text-xl font-bold text-gray-800">旁站记录单登记</h2>
        <p className="text-sm text-gray-500 mt-1">监理员登记入口数据，提交待审核或补正退回的单据</p>
      </div>

      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <StatCard label="待审核" value={stats.byStatus[STATUS.PENDING_REVIEW]} color="blue" />
          <StatCard label="退回补正" value={stats.byStatus[STATUS.RETURNED]} color="orange" />
          <StatCard label="缺料" value={stats.byStatus[STATUS.MATERIAL_MISSING]} color="red" />
          <StatCard label="临期/逾期" value={(stats.byWarningGroup?.approaching || 0) + (stats.byWarningGroup?.overdue || 0)} color="purple" />
        </div>
      )}
      ...
      <div className="card">
        <div className="flex justify-between items-center mb-3">
          ...
          <div className="flex gap-2">
            ...
            {user?.role === ROLES.REGISTRAR && (
              <button className="btn btn-primary" onClick={() => setShowCreate(true)}>+ 新增登记</button>
            )}
          </div>
        </div>
        ...
        <div className="table-wrap">
          <table className="data-table">
            ...
            <tbody>
              {loading && (
                <tr><td colSpan={12} className="empty-state">加载中...</td></tr>
              )}
              {!loading && records.length === 0 && (
                <tr><td colSpan={12} className="empty-state">暂无数据</td></tr>
              )}
              {records.map(r => {
                ...
                <tr key={r.id} className={!canOperate ? 'opacity-60' : ''}>
                  ...
                  <td>{r.projectName}</td>
                  <td>{r.location}</td>
                  <td className="max-w-[200px] truncate" title={r.workContent}>{r.workContent}</td>
      ...
      {detailId && (
        <DetailModal id={detailId} onClose={() => setDetailId(null)} onRefresh={refresh} />
      )}
      {showBatch && (
      ...
      {showCreate && (
        <CreateModal onClose={() => setShowCreate(false)} onSuccess={() => { setShowCreate(false); refresh(); }} />
      )}
    </div>
  );
}

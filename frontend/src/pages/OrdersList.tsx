...
export default function OrdersList() {
  ...
  const getAvailableActions = () => {
    ...
    if (user.role === "registrar") actions.push({ id: "submit", name: "提交" });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
                    ...
                    <div className="mt-2 max-h-48 overflow-y-auto space-y-1">
                      {batchResult.results.map((r: any) => {
                        ...
                        return (
                          ...
                          >
                            <span className="font-medium">{order?.title || r.order_id}</span>
                            <span>{r.reason}</span>
                          </div>
                        );
                      })}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan={showBatch ? 8 : 7} className="p-8 text-center text-gray-500">加载中...</td>
                  </tr>
                ) : orders.length === 0 ? (
                  <tr>
                    <td colSpan={showBatch ? 8 : 7} className="p-8 text-center text-gray-500">暂无入职办理单</td>
                  </tr>
                ) : (
                  orders.map((order) => (
                    <tr
                    ...
                    >
                      ...
                      <td className="p-3">
                        ...
                        <div className="text-xs text-gray-500 mt-0.5">
                          v{order.version} · {new Date(order.due_date).toLocaleDateString("zh-CN")}
                        </div>
                      </td>
                      ...
                      <td className="p-3">
                        <span className={`inline-flex px-2 py-1 rounded text-xs font-medium ${statusColors[order.status] || "bg-gray-100"}`}>
                          {statusNames[order.status] || order.status}
                        </span>
                        {order.is_exception && <span className="ml-2 text-xs text-red-600">异常</span>}
                      </td>
                      <td className="p-3">
                      ...
                      <td className="p-3 text-gray-700">
                        ...
                        <div className="text-xs text-gray-500">{roleNames[order.current_role]}</div>
                      </td>
                    </tr>
                  ))
                )}
        ...
        <div className="mt-4 text-sm text-gray-500">共 {orders.length} 条记录</div>
      </main>
    </div>
  );
}

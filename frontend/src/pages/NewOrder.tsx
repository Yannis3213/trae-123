...
export default function NewOrder() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const data: any = { candidate_name: candidateName, position, department };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-3xl mx-auto px-4 py-6">
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-5">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">候选人姓名 <span className="text-red-500">*</span></label>
            <input
              type="text"
              value={candidateName}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">截止日期（留空默认为 15 天后）</label>
            <input
              type="date"
              value={dueDate}

...
export default function ApplicationList({ user, onViewDetail }: Props) {
  ...
  useEffect(() => {
    loadData();
  }, [statusFilter, urgencyFilter, user.id]);
    ...
    <div style={{ display: 'flex', height: 'calc(100vh - 60px)' }}>
      <div style={{ flex: 1, padding: '20px', overflow: 'auto' }}>
        {statistics && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '20px' }}>
            <div className="card" style={{ padding: '16px' }}>
              ...
              <div style={{ fontSize: '28px', fontWeight: 700, color: '#1e3a8a' }}>{statistics.total}</div>
            </div>
            <div className="card" style={{ padding: '16px' }}>
              <div style={{ fontSize: '13px', color: '#6b7280' }}>待分派</div>
              <div style={{ fontSize: '28px', fontWeight: 700, color: '#92400e' }}>{statistics.pending}</div>
            </div>
            <div className="card" style={{ padding: '16px' }}>
              <div style={{ fontSize: '13px', color: '#6b7280' }}>已转办</div>
              <div style={{ fontSize: '28px', fontWeight: 700, color: '#1e40af' }}>{statistics.transferred}</div>
            </div>
            <div className="card" style={{ padding: '16px' }}>
              <div style={{ fontSize: '13px', color: '#6b7280' }}>
                到期预警
                <span className="badge badge-normal" style={{ marginLeft: '8px' }}>正常 {statistics.urgency.normal}</span>
                <span className="badge badge-warning" style={{ marginLeft: '4px' }}>临期 {statistics.urgency.warning}</span>
                <span className="badge badge-overdue" style={{ marginLeft: '4px' }}>逾期 {statistics.urgency.overdue}</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="card" style={{ marginBottom: '20px' }}>
          ...
          {batchResults && (
            <div className="card" style={{ marginBottom: '16px', background: '#f9fafb', border: '1px solid #e5e7eb' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                <h3 style={{ fontSize: '15px' }}>📋 批量处理结果</h3>
                <button className="btn btn-secondary" onClick={() => setBatchResults(null)} style={{ fontSize: '12px', padding: '4px 10px' }}>关闭</button>
              </div>
                ...
                <tbody>
                  {batchResults.map((r) => (
                    <tr key={r.application_id}>
                      ...
                      <td style={{ fontFamily: 'monospace', fontSize: '12px' }}>{r.application_id.slice(0, 8)}...</td>
                      <td>
                        {r.success ? (
          ...
          <table>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={11} style={{ textAlign: 'center', padding: '40px', color: '#6b7280' }}>加载中...</td></tr>
              ) : applications.length === 0 ? (
                <tr><td colSpan={11} style={{ textAlign: 'center', padding: '40px', color: '#6b7280' }}>暂无数据</td></tr>
              ) : applications.map((app) => (
                <tr key={app.id} style={{ background: selectedId === app.id ? '#eff6ff' : undefined }}>
                  <td>
                    <input
                      type="checkbox"
                  ...
                  <td>
                    {app.urgency === 'overdue' && (
                      <span className="badge badge-overdue" style={{ marginRight: '4px' }}>⚠ 超时</span>
                    )}
      ...
      <div style={{ width: '340px', background: '#f9fafb', borderLeft: '1px solid #e5e7eb', padding: '20px', overflow: 'auto' }}>
        ...
        {!selectedId ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div className="card" style={{ padding: '14px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                {selectedEvidence.materials_ok ? (
                  <span className="badge badge-visited">✓ 齐全 ({selectedEvidence.materials_count}份)</span>
                ) : (
            ...
            <div className="card" style={{ padding: '14px', background: selectedEvidence.all_complete ? '#f0fdf4' : '#fef2f2' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ marginTop: '24px', padding: '16px', background: '#eff6ff', borderRadius: '8px' }}>
          ...
          <ul style={{ fontSize: '12px', color: '#374151', paddingLeft: '18px', lineHeight: 1.8 }}>
            <li>勾选左侧单据可批量处理</li>
            <li>点击表头可按状态/预警筛选</li>
            <li>逾期单据会高亮显示责任人</li>
            <li>右上角可快速切换角色</li>
